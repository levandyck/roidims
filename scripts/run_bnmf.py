...
def run_bnmf(subjects: list, rois: list, n_runs: int=100, r_thresh: float=0.3):
    ...
    _ = submit_parallel_jobs(
        find_consensus_dims, args,
        joblib_kwargs={"n_jobs": len(subjects)*len(rois), "verbose": 10},
    )

    # Find consistent dimensions across subjects
    args = [(subjects, roi, r_thresh) for roi in rois]
    _ = submit_parallel_jobs(
        find_consistent_dims, args,
        joblib_kwargs={"n_jobs": len(rois), "verbose": 10},
    )
